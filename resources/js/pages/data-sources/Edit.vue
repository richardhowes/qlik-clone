    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 p-4">
            <div>
                ...
                <p class="text-muted-foreground">Update your {{ formatType(dataSource.type) }} connection settings</p>
            </div>

            <Card class="max-w-2xl">
                <CardHeader>
                    <CardTitle>Connection Details</CardTitle>
                    <CardDescription> Modify your database connection settings </CardDescription>
                </CardHeader>
                <CardContent>
                    <form @submit.prevent="submit" class="space-y-6">
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <ConnectionForm v-model="form.connection_config" :type="dataSource.type" :errors="form.errors" />
                            </div>
                        </div>
                        ...
                        <div class="flex justify-end gap-2">
                            <Button type="button" variant="outline" @click="router.visit(route('data-sources.show', dataSource.id))"> Cancel </Button>
                            <Button type="button" variant="outline" @click="testConnection" :disabled="testing || !form.name">
                                {{ testing ? 'Testing...' : 'Test Connection' }}
                            </Button>
                            <Button type="submit" :disabled="form.processing"> Update Data Source </Button>
                        </div>
                    </form>
                </CardContent>
            </Card>
